...
@Injectable()
export class GetTokenRefreshGuard extends AuthGuard("get-token-refresh") {
    ...
    public handleRequest(error: unknown, user: any, info: unknown) {
        if(error) {
            ...
            if(info) {
                throw this.exceptionService.getUnauthorizedException("Invalid refresh token was provided")
            }
            else {
                if(user) {
                    return user
