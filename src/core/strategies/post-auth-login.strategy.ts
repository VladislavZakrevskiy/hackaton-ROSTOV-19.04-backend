...
export class PostAuthLoginStrategy extends PassportStrategy(Strategy, "post-auth-login") {
    ...
    public async validate(login: string, password: string) {
        ...
        if(user) {
            const result = await this.bcryptService.compareDataWithEncrypted(password, user.password)

            if(result) {
                return user
            }
            else {
                throw this.exceptionService.getForbiddenException("Bad password was provided")
            }
        }
        else {
